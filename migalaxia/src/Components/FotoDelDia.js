		...
		const fetchDataForPosts = async () => {
			try {
				...
				if (datosImagen.media_type === "image") {
					...
					setBoton(<a className="btn btn-secondary my-3" href={datosImagen.url} download={nombreArchivo}>Descargar foto</a>);
					setImagen(<img src={datosImagen.url} className="shadow rounded float-center img-fluid" alt="La imagen astronómica de hoy"/>);
				} else {
					setTipo("Vídeo");
					setBoton(<a className="btn btn-outline-secondary my-3 disabled text-wrap" tabIndex="-1" aria-disabled="true">No se pueden descargar vídeos</a>);
					setImagen(<div className="ratio ratio-16x9">
						<iframe title="Vídeo del día" src={datosImagen.url} alt="La imagen astronómica de hoy"/>
					</div>);
				}
			} catch (err) {
				setImagen(<h3>{MENSAJE_ERROR}</h3>);
	...
	return (
		<>
			<div className="row">
				...
				<div className="col text-center">{boton}</div>
			</div>
		</>
	);
}
